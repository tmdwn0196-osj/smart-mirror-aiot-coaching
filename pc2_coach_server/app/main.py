...
from app.schemas import CoachingResponse, ExerciseBaselineCreateRequest, ExerciseBaselineRecord, FeaturePayload
from app.services import create_baseline_record, generate_coaching_response
...
def health() -> dict:
    ...
    fallback_llm = check_fallback_llm_health()
    llm_ready = primary_llm["status"] == "ok" or fallback_llm["status"] == "ok"
...
def exercise_baseline(user_id: str, exercise_type: str | None = Query(default=None)) -> dict:
    baseline = get_latest_baseline(user_id, exercise_type.lower() if exercise_type else None)
    if baseline is None:
