...
| `available_days_per_week` | 주당 운동 가능 횟수 | 1~7 허용. PC2는 응답 `weekly_routine` 일수가 이 값을 초과하면 파싱 실패로 처리함 |
...
| `weekly_routine` | 일자별 루틴 배열 | 최소 1일 이상이어야 하며, 일수는 `available_days_per_week`를 초과할 수 없음 |
...
| `pc3_payload` | 프론트 전달용 payload | PC2 parser가 top-level 루틴 결과와 요청의 `available_days_per_week`, `restricted_body_parts`를 기준으로 일관되게 재구성함 |
